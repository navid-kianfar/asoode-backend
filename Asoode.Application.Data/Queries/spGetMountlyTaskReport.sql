...
-- =============================================
CREATE PROCEDURE spGetMountlyTaskReport
...
BEGIN
	...
	DECLARE @Date DATE = CONVERT(DATE, DATEADD(DAY, -90, GETDATE()))

    -- Insert statements for procedure here
	INSERT INTO @t
SELECT ID, CONVERT(DATE,a.CreatedAt), [Type] FROM activities a
...
SELECT a.[Date] [Date], COUNT(1) [Count] FROM @t a WHERE a.[Date] > @Date AND (a.[Type] = 623 OR a.[Type] = 626 OR a.[Type] = 627)
GROUP BY CONVERT(DATE, a.[Date])

SELECT a.[Date] [Date], COUNT(1) [Count] FROM @t a WHERE a.[Date] > @Date AND (a.[Type] = 602 OR a.[Type] = 628 OR a.[Type] = 629)
GROUP BY CONVERT(DATE, a.[Date])

SELECT a.[Date] [Date], COUNT(1) [Count] FROM @t a WHERE a.[Date] > @Date AND (a.[Type] = 600)
GROUP BY CONVERT(DATE, a.[Date])
END