        ...
        ProjectCreateViewModel model)
    {
        try
        {
            ...
            var alreadyCreated = await unit.Projects
                ...
                .Select(i => new { i.Id, i.Complex, i.Premium })
                .ToArrayAsync();
        ...
        try
        {
            using (var unit = _serviceProvider.GetService<ProjectManagementDbContext>())
            {
                ...
                if (attachments.Any())
                {
                    ...
                    foreach (var attachment in attachments) await storageService.Delete(attachment.Attachment.Path);
            ...
            using (var unit = _serviceProvider.GetService<ProjectManagementDbContext>())
            {
                ...
                foreach (var set in reportResultSets["Table-1"])
                {
                    ...
                    if (!mapped.ContainsKey(date))
                        mapped.Add(date, new ProjectProgressViewModel { Date = DateTime.Parse(date) });
