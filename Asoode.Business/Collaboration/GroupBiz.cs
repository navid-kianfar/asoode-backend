...
#pragma warning restore 4014
    ...
    public async Task<OperationResult<bool>> AddAccess(Guid userId, Guid groupId, AccessViewModel model)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<CollaborationDbContext>())
            {
                if (!model.Members.Any()) return OperationResult<bool>.Success();
                ...
                var tasks = await (from task in unit.WorkPackageTasks
                    ...
                    where packageIds.Contains(task.PackageId) &&
                          ...
                          task.State != WorkPackageTaskState.Duplicate && (
                              (task.DueAt.HasValue && task.DueAt.Value >= timeOff.BeginAt &&
                               task.DueAt.Value <= timeOff.EndAt) ||
                              (task.BeginAt.HasValue && task.BeginAt.Value >= timeOff.BeginAt &&
                               task.BeginAt.Value <= timeOff.EndAt) ||
                              (task.EndAt.HasValue && task.EndAt.Value >= timeOff.BeginAt &&
                               task.EndAt.Value <= timeOff.EndAt)
                          )
        ...
        try
        {
            using (var unit = _serviceProvider.GetService<CollaborationDbContext>())
            {
                ...
                var permission = await unit.GroupMembers
                    .AsNoTracking()
                    .SingleOrDefaultAsync(
                        i => i.UserId == userId && i.GroupId == timeOff.GroupId && i.DeletedAt == null);
