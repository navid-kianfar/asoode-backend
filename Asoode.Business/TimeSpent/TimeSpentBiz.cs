...
namespace Asoode.Business.TimeSpent;
...
internal class TimeSpentBiz : ITimeSpentBiz
{
        ...
        try
        {
            using (var unit = _serviceProvider.GetService<ProjectManagementDbContext>())
            {
                ...
                if (user == null || user.IsLocked || user.Blocked)
                    return OperationResult<TimeSpentViewModel[]>.Rejected();
    ...
    public async Task<OperationResult<TimeSpentViewModel[]>> GroupTimeSpents(Guid userId, Guid groupId,
