...
internal class MessengerBiz : IMessengerBiz
{
    ...
    public async Task<OperationResult<UploadResultViewModel>> AddAttachment(Guid userId, Guid channelId,
        StorageItemDto file)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<CollaborationDbContext>())
            {
                ...
                file.UserId = userId;
                file.PlanId = planId;
                file.Section = UploadSection.Messenger;
                file.RecordId = channelId;
