...
internal class AccountBiz : IAccountBiz
{
    ...
    public async Task<OperationResult<bool>> ChangePicture(Guid userId, StorageItemDto file)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<AccountDbContext>())
            {
                ...
                file.Section = UploadSection.UserAvatar;
                file.PlanId = Guid.Empty;
                file.RecordId = user.Id;
                file.UserId = user.Id;
                var upload = await storageService.Upload(file);
    ...
    public async Task<OperationResult<RegisterResultViewModel>> Register(RegisterRequestViewModel model)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<AccountDbContext>())
            {
                ...
                if (!isPhone && !isEmail)
                    throw new Exception($"('{username}' => '{model.Username}') is not a valid username");
                var phone = isPhone ? NormalizePhone(model.Username) : null;
    ...
    public async Task<OperationResult<bool>> RemovePicture(Guid userId)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<AccountDbContext>())
            {
                ...
                if (user == null) return OperationResult<bool>.NotFound();
                if (!string.IsNullOrEmpty(user.Avatar))
                    await _serviceProvider.GetService<IStorageService>()
                        .Delete(user.Avatar);
    ...
    public async Task<OperationResult<bool>> UpdateProfile(
        Guid userId, UserProfileUpdateViewModel model, StorageItemDto avatar)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<AccountDbContext>())
            {
                ...
                if (avatar != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(user.Avatar)) await storageService.Delete(user.Avatar);

                    avatar.Section = UploadSection.UserAvatar;
                    avatar.UserId = userId;
                    avatar.RecordId = userId;
                    avatar.PlanId = Guid.Empty;

                    var op = await storageService.Upload(avatar);
                    if (op.Status == OperationResultStatus.Success)
                    {
                        await storageService.Delete(user.Avatar);
                        user.Avatar = op.Data.Url;
                    }
                }
        ...
        GridFilterWithParams<GridQuery> model)
    {
        try
        {
            using (var unit = _serviceProvider.GetService<AccountDbContext>())
            {
                ...
                foreach (var item in result.Data.Items) item.Plan = plans.First(p => p.UserId == item.Id).ToViewModel();
