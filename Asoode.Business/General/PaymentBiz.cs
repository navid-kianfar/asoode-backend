...
internal class PaymentBiz : IPaymentBiz
{
    ...
    {
        var unit = (AccountDbContext)unitObj;
        var order = (Order)orderObj;
        ...
        var hasExpireDate = oldPlan.ExpireAt.HasValue && oldPlan.ExpireAt.Value > now;
        var mustRefund = order.OrderType == OrderType.Change && hasExpireDate && oldPlan.Type != PlanType.Free;
    ...
    private async Task<T> CallApi<T>(string apiUrl, object value, bool isToken = false) where T : class
    {
        ...
        var client = new RestClient(baseHost);
