    ...
    public static void Main(string[] args)
    {
        ...
        using (var scope = host.Services.CreateScope())
        {
            try
            {
                ...
                {
                    serverInfo.ContentRootPath = Path.Combine(serverInfo.RootPath, "../backend/wwwroot");
                    serverInfo.FilesRootPath = Path.Combine(serverInfo.ContentRootPath, "storage");
                    serverInfo.I18nRootPath = Path.Combine(serverInfo.RootPath, "../backend/I18n");
                    serverInfo.EmailsRootPath = Path.Combine(serverInfo.RootPath, "../backend/templates/email");
                    serverInfo.SmsRootPath = Path.Combine(serverInfo.RootPath, "../backend/templates/sms");
                }
            }
            catch (Exception ex)
            {
        ...
        return Host.CreateDefaultBuilder(args)
            .ConfigureWebHostDefaults(webBuilder =>
            {
                webBuilder.ConfigureAppConfiguration((hostingContext, cfg) =>
                    ...
                    .UseKestrel(options => { options.Listen(IPAddress.Parse(ip), httpPort); })
                    .UseStartup<Startup>();
            }).Build();
    }
}